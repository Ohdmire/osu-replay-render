...
| `--encoder <x264\|x265\|nvenc>` | 视频编码器：libx264（默认）/ libx265（preset medium + crf）或 NVENC 硬件编码（bgr0 直喂 + p5/hq/vbr/cq，**端到端约比 x264 快 1.7×、比 x265 快 3.2×**） |
...
- **圆**：四层渐变圆身（外填充/外渐变/内渐变/内填充）、白描边、combo
  数字（Exo 2Pro Bold）、approach circle、命中动画复刻
...
`assets/`：
- `fonts/Exo 2Pro-*.ttf` — 文字渲染（ab_glyph 启动时按 24/48/96 三档
  em 栅格化进图集）。
