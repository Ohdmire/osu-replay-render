...
const EXO2_BOLD_TTF: &[u8] = include_bytes!("../assets/fonts/Exo 2Pro-Bold.ttf");
const EXO2_SEMIBOLD_TTF: &[u8] = include_bytes!("../assets/fonts/Exo 2Pro-SemiBold.ttf");
...
fn build_atlas(bg_image: Option<Image>) -> (Atlas, TtfFont, TtfFont) {
    let (mut bold, mut bold_images) = TtfFont::rasterize(EXO2_BOLD_TTF, true);
    let (mut semibold, mut semibold_images) = TtfFont::rasterize(EXO2_SEMIBOLD_TTF, false);
...
struct Options {
    ...
    classic_score: bool,
    skin: String,
    /// auto | nvenc | x264 (auto probes NVENC, falls back to x264).
    encoder: String,
    /// cq (nvenc) / crf (x264); 18 by default.
    quality: u32,
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--encoder" => {
                ...
                if !matches!(enc.as_str(), "auto" | "nvenc" | "x264") {
                    return Err("--encoder must be auto, nvenc or x264".into());
                }
                opts.encoder = enc;
            }
            "--quality" => {
...
fn main() {
    ...
    // Resolve the encoder: auto probes NVENC with a tiny test encode.
    let encoder = if opts.encoder == "auto" {
        let probe = std::process::Command::new("ffmpeg")
            ...
            .status();
        if probe.map(|s| s.success()).unwrap_or(false) { "nvenc" } else { "x264" }
    } else {
        opts.encoder.as_str()
    };
    ...
    let mut output = if let Some(dir) = &opts.png_dir {
        ...
        // does the colour conversion in hardware (no CPU swscale); the x264
        // path converts to yuv420p on the CPU as before.
